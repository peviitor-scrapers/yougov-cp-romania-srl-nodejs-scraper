...
This scraper extracts job listings from the YouGov Workday Careers API and imports them to peviitor.ro.

Target: https://yougov.wd103.myworkdayjobs.com
...
1. **Start with brand** - We know the brand ("YouGov")
...
8. **Scrape new jobs** - Extract jobs from YouGov Workday API (Romania)
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts YouGov Workday jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from YouGov Workday API (Romania only)
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape YouGov Workday API (jobs for Romania)
    │
    ▼
...
- **YouGov Workday API**: `https://yougov.wd103.myworkdayjobs.com/wday/cxs/yougov/YouGov_External_Careers/jobs` — POST with JSON body (limit, offset)
